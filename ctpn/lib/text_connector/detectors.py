...
class TextDetector:
    ...
    def detect(self, text_proposals, scores, size):
        ...
        # 删除得分较低的proposal
        keep_index = np.where(scores > TextLineCfg.TEXT_PROPOSALS_MIN_SCORE)[0]
        text_proposals, scores = text_proposals[keep_index], scores[keep_index]
        ...
        # 对proposal做nms
        keep_index = nms(np.hstack((text_proposals, scores)), TextLineCfg.TEXT_PROPOSALS_NMS_THRESH)
        text_proposals, scores = text_proposals[keep_index], scores[keep_index]

        # print('========TextDetector detect', text_proposals)

        # 获取检测结果
        text_recs = self.text_proposal_connector.get_text_lines(text_proposals, scores, size)
        keep_index = self.filter_boxes(text_recs)
        return text_recs[keep_index]
